		...
		function __construct(){
			...
			if(isset($_GET['case']) && $_GET['case'] == 'verifyUser'){
				if(!isset($_GET['summoner']) || !isset($_GET['region'])){
					header('Location: '.$this->conf->http_url.'finder/');
				}
				...
				if($summonerId){
					...
					if(!isset($_COOKIE['champions'])){
						...
						if(isset($data['error'])){
							...
							header('Location: '.$this->conf->http_url.'finder/');exit;
						}
						if(count($data) < 10){
					...
					setcookie("tries",5);
					setcookie("level",1);
					header('Location: '.$this->conf->http_url.'finder/?game');
			...
			if($_SERVER['REQUEST_METHOD'] == 'POST'){
				...
				switch ($case) {
					case 'index':
						...
						setcookie("tries",false,time()-1);
						setcookie("level",false,time()-1);
						require('events/summonerForm.php');
						break;
					case 'game':
						if(!isset($_COOKIE['summoner']) || !isset($_COOKIE['champions']) || !isset($_COOKIE['score']) || !isset($_COOKIE['tries']) || !(isset($_COOKIE['level'])) ){
							http_response_code(400);exit;
						}
					...
					case 'submitScore':
						...
						$scores = json_decode(file_get_contents($this->conf->http_url.'src/json/finder-scores.json',false,$context),true);
						...
						$fo = fopen($this->conf->disk_url.'src/json/finder-scores.json','w+');
					...
					case 'scores':
						...
						$this->scores = json_decode(file_get_contents($this->conf->http_url.'src/json/finder-scores.json',false,$context),true);
						require('events/scores.php');
						break;
					default:
